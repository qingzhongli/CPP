...
float dotproduct_avx2(const float * p1, const float * p2, size_t n) {
    ...
    float sum[8] = {0};
    // single-precision (32-bit)  * 8  = 256
    __m256 a, b;
    __m256 c = _mm256_setzero_ps();

    for (size_t i = 0; i < n; i+=8) {
        // Load data from memory to registers
        // https://www.intel.com/content/www/us/en/docs/intrinsics-guide/index.html#text=_mm256_load_ps
        a = _mm256_load_ps(p1 + i);
        b = _mm256_load_ps(p2 + i);

        // https://www.intel.com/content/www/us/en/docs/intrinsics-guide/index.html#text=_mm256_add_ps
        // https://www.intel.com/content/www/us/en/docs/intrinsics-guide/index.html#text=_mm256_mul_ps
...
#ifdef WITH_NEON
    if (n % 4 != 0) {
        std:cerr << "The size n must be a multiple of 4." << endl;
        return 0.0f;
    }
